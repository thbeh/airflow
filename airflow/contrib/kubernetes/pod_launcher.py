"""
Utilities to work with kubernetes
...
def incluster_namespace():
    ...
    """
    config.load_incluster_config()
    api_key = str(base64.b64decode(config.incluster_config.configuration.api_key['authorization'].split(' ')[-1]))
...
class KubernetesLauncher:
    ...
    def __init__(self, kube_object, request_factory):
        ...
        if not isinstance(request_factory, KubernetesRequestFactory):
            raise Exception('`request_factory` must inherit from KubernetesRequestFactory')
    ...
    def _execution_finished(self):
        k8s_beta = kube_client()
        resp = k8s_beta.read_namespaced_pod_status(self.pod.name, namespace=self.pod.namespace)
...
class KubernetesCommunicationService:
    ...
    def pod_return_data(self, task_id):
        """
            ...
            return data to etcd. If the return_data_file is generated by the application, the
            pre stop hook will upload it to etcd and we will be download it back to airflow.
        """
        ...
        try:
            ...
            logging.info('result for querying {} for task id {}: {}'.format(self.url, task_id, result))
    ...
    def from_dag_default_args(dag):
        ...
        logging.info('Setting etcd endpoint from dag default args {}:{}'.format(etcd_host, etcd_port))
        if not etcd_host:
            raise Exception('`KubernetesCommunicationService` requires etcd endpoint. Please defined it in dag '
                            'degault_args')
